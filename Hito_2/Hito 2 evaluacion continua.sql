...
select u.fullname, bo.editorial, (count(pages))as IBRANI_90
from book as bo
inner join prestamos as p on bo.id_book = p.id_book
inner join users u on p.id_user = u.id_user
where bo.pages > 90;

create or replace function Mayor_a_90 (editorial varchar(50), cantidad int)
returns int
begin
    declare respuesta int;
    select(count(pages))as IBRANI_90
    into respuesta
from book as bo
inner join prestamos as p on bo.id_book = p.id_book
inner join users u on p.id_user = u.id_user
where bo.pages > cantidad and bo.editorial=editorial;
    return respuesta;
end;
select Mayor_a_90('INBRANI',90)as IBRANI_90;

select concat (cat.type, lib.editorial ) as DESCRIPTION,    lib.pages as PAGES
from book as lib
inner join prestamos as pres on lib.id_book = pres.id_book
inner join users as us on us.id_user = pres.id_user
inner join category as cat on cat.id_book = lib.id_book
inner join users as us on us.id_user = pres.id_user
where cat.type = 'MANGA' and lib.editorial = 'IBRANI';

create or replace function Paginas_pares_impares( numeroPag int)
returns int
begin
    declare respuesta varchar(100);
    declare algo varchar (24);
        if (numeroPag%2=0) then
             set respuesta = 'PAR';
            set algo=concat(respuesta,numeroPag);
